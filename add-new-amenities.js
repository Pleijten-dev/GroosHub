...
// Map category to existing format (simplified - using input category for now)
function getCategoryKey(category) {
  const map = {
    'Wellness & Recreatie': 'ontspanning_en_vrije_tijd',
    'Cafés en avond programma': 'sociaal_en_gastvrij',
    'Groen & Recreatie': 'ontspanning_en_vrije_tijd',
    'Kinderopvang & Opvang': 'gezin_en_kindvriendelijk',
    'Cultuur & Entertainment': 'cultuur_en_educatie',
    'Sport faciliteiten': 'gezondheid',
    'Zorg (Huisarts & Apotheek)': 'gezondheid',
    'Zorg (Paramedische voorzieningen)': 'gezondheid',
    'Winkels (Dagelijkse boodschappen)': 'praktisch',
    'Winkels (Overige retail)': 'praktisch',
    'Mid-range Restaurants (€€€)': 'food',
    'Budget Restaurants (€)': 'food',
    'Upscale Restaurants (€€€€-€€€€€)': 'sociaal_en_gastvrij',
    'Onderwijs (Basisschool)': 'cultuur_en_educatie',
    'Onderwijs (Voortgezet onderwijs)': 'cultuur_en_educatie',
    'Onderwijs (Hoger onderwijs)': 'cultuur_en_educatie'
  };
  return map[category] || 'ontspanning_en_vrije_tijd';
}
...
console.log('ADDING NEW AMENITIES');
console.log('='.repeat(80));

let newCommunalCount = 0;
let newPublicCount = 0;
...
rows.forEach(row => {
  ...
  const newSpace = {
    ...
    name: row.title,
    scale: scaleMap[row.scale] || 'middenschaal',
    category: getCategoryKey(row.category),
    target_groups: targetGroups
  };

  if (row.type === 'communal') {
    // Check if already exists
    const exists = communal.nl.spaces.find(s => s.name === row.title);
    if (!exists) {
      communal.nl.spaces.push(newSpace);
      newCommunalCount++;
      console.log(`✅ Added communal: ${row.title}`);
    }
  } else if (row.type === 'public') {
    const exists = publicSpaces.nl.spaces.find(s => s.name === row.title);
    if (!exists) {
      publicSpaces.nl.spaces.push(newSpace);
      newPublicCount++;
      console.log(`✅ Added public: ${row.title}`);
    }
  }
});
...
console.log('SUMMARY');
console.log('='.repeat(80));
console.log(`New communal spaces added: ${newCommunalCount}`);
console.log(`New public spaces added: ${newPublicCount}`);
console.log(`Total new amenities: ${newCommunalCount + newPublicCount}`);
...
console.log('='.repeat(80));
console.log(`Total communal spaces: ${communal.nl.spaces.length}`);
console.log(`Total public spaces: ${publicSpaces.nl.spaces.length}`);
console.log(`Grand total: ${communal.nl.spaces.length + publicSpaces.nl.spaces.length}`);
console.log('='.repeat(80));
